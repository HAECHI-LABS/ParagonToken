    ...
    ERC20Burnable
{
    ...
    string constant private _symbol = "PGR";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "PGR/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "PGR/transferFrom : Should not send to zero address"
        );
        ...
        _approve(
            ...
            _allowances[from][msg.sender].sub(
                amount,
                "PGR/transferFrom : Cannot send more than allowance"
            )
        );
        success = true;
    }
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "PGR/approve : Should not approve zero address"
        );
